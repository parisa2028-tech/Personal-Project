...
# Intro & Tutorial
def tutorial():
    pygame.display.set_caption("Tutorial (press the red 'x' in the top left to exit)")
...
# Main Game
def play():
    pygame.display.set_caption("Main Game (press the red 'x' in the top left to exit)")
    ...
    while run:
        ...
        # Events
        # When the x button is clicked, the window will close
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
        ...
        if bird.at_target and pygame.sprite.spritecollide(bird, all_pillars, False,
                                                          collided=pygame.sprite.collide_rect_ratio(0.4)):
            window.blit(scaled_game_over, (20, 70))
            pygame.display.update()  # refresh screen so it's visible
            # Waits for 0.5 seconds
            pygame.time.delay(500)  # not like time.sleep bc it still lets pygame update
...
# Sandbox
def sandbox():
    pygame.display.set_caption("Sandbox (press the red 'x' in the top left to exit)")
